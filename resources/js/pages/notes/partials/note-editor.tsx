...
type Props = {
  ...
  onSave: (payload: { title: string; content: string; folder_id: number | null }) => void
  onCancel: () => void
  saving?: boolean
}
...
function flattenFolders(folders: Folder[], depth = 0, acc: FolderOption[] = []) {
  for (const f of folders) {
    ...
    const indent = "\u00A0".repeat(depth * 3) // 3 espacios no-break
...
export function NoteEditor({ note, folderId, folders, onSave, onCancel, saving }: Props) {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            {options.map((opt) => (
              <SelectItem key={String(opt.id)} value={opt.id === null ? "null" : String(opt.id)}>
                {opt.label}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <div className="space-y-2">
        ...
        {/* ✅ dark */}
        <div data-color-mode="dark" className="rounded-md border overflow-hidden">
